...
////  * РазмерПачкиЧтения - Число - количество байт, получаемое из очереди за одно обращение
...
//Функция ПолучитьСообщения(Группа, Подписчик, РазмерПачкиЧтения, СтруктураНастроекСоединения = Неопределено) Экспорт 		
//	Возврат осОчередиСообщенийСервер.ПолучитьСообщения(Группа, Подписчик, РазмерПачкиЧтения, СтруктураНастроекСоединения);   	
//КонецФункции
...
////   * Смещение - Число - смещение чтения
////   * Сообщение - Строка - сообщение
//Функция ПолучитьСообщенияПоСоединению(Группа, Подписчик, РазмерПачкиЧтения, Соединение) Экспорт 		
//	Возврат осОчередиСообщенийСервер.ПолучитьСообщенияПоСоединению(Группа, Подписчик, РазмерПачкиЧтения, Соединение);   	
//КонецФункции
...
Функция ПолучитьСтруктуруОписанияОчереди() Экспорт
	...
	Возврат Новый Структура("ИмяОчереди, Наименование, Тип, РазмерПачкиЧтения, Ссылка");
...
Функция ПолучитьОписаниеОчередиПоСсылке(Очередь) Экспорт
	...
	|	осОчередиСообщений.РазмерПачкиЧтения КАК РазмерПачкиЧтения,
...
Функция ПолучитьОписаниеВнешнегоМетода(Метод) Экспорт
	...
	ИначеЕсли Метод = Перечисления.осВнешниеМетодыОбменаСообщениями.ПрочитатьСообщения Тогда
		...
		ОписаниеМетода.Вставить("Группа","");
		ОписаниеМетода.Вставить("Подписчик","");
		ОписаниеМетода.Вставить("РазмерПачкиЧтения",0);
...
Функция ЗаполнитьСвойстваВнешнегоМетода(ОписаниеМетода)
	...
	ИначеЕсли Метод = Перечисления.осВнешниеМетодыОбменаСообщениями.ПрочитатьСообщения Тогда
		...
		АдресРесурса = СтрЗаменить(АдресРесурса, "%1", ОписаниеМетода.Группа);
		АдресРесурса = СтрЗаменить(АдресРесурса, "%2", ОписаниеМетода.Подписчик);
		Если ОписаниеМетода.РазмерПачкиЧтения > 0 Тогда
...
Процедура ОбработатьОчередиСообщений() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	осПравилаОбработки.Очередь.РазмерПачкиЧтения КАК РазмерПачкиЧтения,
	...
	Пока Выборка.Следующий() Цикл
		...
		ПараметрыПотока.Вставить("РазмерПачкиЧтения",Выборка.РазмерПачки);
...
Функция ОбработкаСобытийПрочитатьСообщения(СистемныеПараметры, Соединение)
	...
	ОписаниеВнешнегоМетода.РазмерПачкиЧтения = СистемныеПараметры.РазмерПачкиЧтения;
...
Процедура ЗарегистрироватьОшибку(СообщениеОбОшибке, СистемныеПараметры)
	...
	Метаданные.Подсистемы.осОчередиСообщений, 
	, 
	КомментарийЗаписи);
