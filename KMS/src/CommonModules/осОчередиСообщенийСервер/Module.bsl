...
Функция ТекстНовогоПроизвольногоКодаДействия() Экспорт
	...
	Возврат
	"Правило действия описывается на языке 1С в случае необходимости
	|обработки результатов фильтрации. В случае отсутствия такой
	|необходимости - использовать настроку ""Не обрабатывать"". В
	|этом случае сообщения, прошедшие правила фильтрации будут
	|помещены в очередь действий без постобработки этой очереди.
	|Формирование шаблонов возможно двумя способами: описание
	|кода в справочнике ""Шаблоны действий"" и описанием методов
	|в модуле ""осШаблоныДействийПереопределяемый"". Только 
	|методы, описанные в режиме конфигуратора могут содержать
